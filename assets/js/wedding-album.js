/**
 * ALBUM MATRIMONIALE CON ANIMAZIONE REALISTICA
 * Simula l'apertura di un vero album di matrimonio con pagine che si voltano
 */
...
(function() {
    ...
    class WeddingPhotoAlbum {
        ...
        init() {
            console.log('📖 Inizializzando album matrimoniale...');
            this.createAlbumStructure();
            this.loadPhotos();
            this.overrideOpenFunction();
            
            // Aggiungi fallback globale per ESC key
            document.addEventListener('keydown', (e) => {
                if (e.key === 'Escape' && this.isOpen) {
                    console.log('🔘 ESC premuto - Chiudendo album');
                    this.closeAlbum();
                }
            });
            
            // Carica le foto nella galleria principale con retry
            this.tryLoadGallery();
        }
        ...
        tryLoadGallery() {
            ...
            const attemptLoad = () => {
                attempts++;
                console.log(`🔄 Tentativo ${attempts} di caricamento galleria...`);
                
                const galleryContainer = document.getElementById('photo-gallery');
                if (galleryContainer && this.photos.length > 0) {
                    console.log('✅ Galleria trovata, caricando foto...');
                    this.loadPhotosInMainGallery();
                } else if (attempts < maxAttempts) {
                    console.log(`⏳ Galleria non ancora pronta, riprovo in 500ms...`);
                    setTimeout(attemptLoad, 500);
                } else {
                    console.error('❌ Impossibile caricare la galleria dopo', maxAttempts, 'tentativi');
                }
            };
        ...
        loadPhotos() {
            if (typeof anitaPhotos !== 'undefined' && Array.isArray(anitaPhotos)) {
                // Clona l'array e lo randomizza per varietà nell'album
                this.photos = [...anitaPhotos].filter(photo => photo.filename);
                
                // Randomizzazione Fisher-Yates per mescolare le foto
                for (let i = this.photos.length - 1; i > 0; i--) {
                    const j = Math.floor(Math.random() * (i + 1));
                    [this.photos[i], this.photos[j]] = [this.photos[j], this.photos[i]];
                }
                
                this.totalPages = Math.ceil(this.photos.length / 2); // 2 foto per pagina aperta
        ...
        // Carica le foto nella galleria principale
        loadPhotosInMainGallery() {
            console.log('🖼️ Caricando foto nella galleria principale...');
            
            const galleryContainer = document.getElementById('photo-gallery');
            const loadingGallery = document.getElementById('loading-gallery');
            
            console.log('Gallery container:', galleryContainer);
            console.log('Loading gallery:', loadingGallery);
            console.log('Photos available:', this.photos.length);
            
            if (!galleryContainer) {
                console.error('❌ Elemento photo-gallery non trovato!');
                return;
            }
            
            if (this.photos.length === 0) {
                console.warn('⚠️ Nessuna foto disponibile per la galleria');
                return;
            }
